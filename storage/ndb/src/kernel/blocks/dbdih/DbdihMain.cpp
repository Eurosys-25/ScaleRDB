...
// A variable (isNode_participating_num) is used to handle the Slave instance ID in a round-robin manner.

static Uint32 isNode_participating_num = 11; //normal node. 
...
/*
 * In our design, ScaleRDB has a single master and a single slave that independently handle transactions and checkpoint operations. Consequently, selectedNode1 (master instance) and selectedNode2 (slave instance) receive an Instance ID value and pass the transaction tasks to their respective database instances.
 *
 * Additionally, after the transaction operations are completed, each instance can send an acknowledgment to the master instance using the same macro.
 */
...
void Dbdih::sendINCL_NODEREQ(Signal* signal, Uint32 nodeId, Uint32 extra)
{
  ...
  printf("KCJ: Dbdih::sendINCL_NODEREQ: startNode: %llu, failNr: %llu\n", c_nodeStartMaster.startNode, c_nodeStartMaster.failNr);
...
{
    printf("KCJ: Dbdih::sendMASTER_LCPREQ: nodeId : %llu", nodeId);
  BlockReference ref = calcDihBlockRef(nodeId);
...
//KCJ
  //cmasterdihref = calcDictBlockRef(11);
  ...
  case CopyGCIReq::LOCAL_CHECKPOINT: {
    ok = true;
    jam();
    //cmasterdihref = 16121866;
    printf("KCJ: case CopyGCIReq::LOCAL_CHECKPOINT : %llu\n", cmasterdihref);
    //cmasterdihref = calcDihBlockRef(10);
    c_lcpState.setLcpStatus(LCP_COPY_GCI, __LINE__);
    c_lcpState.m_masterLcpDihRef = cmasterdihref;
    //c_lcpState.m_masterLcpDihRef = calcDihBlockRef(11);
    setNodeActiveStatus();
    break;
  }
  ...
  case ZNDB_SPH5:
    ...
    case NodeState::ST_INITIAL_NODE_RESTART:
      jam();
      {
       ...
       // req->senderRef = calcDihBlockRef(10);
        req->senderRef = reference();
        req->senderData = RNIL;
...
Dbdih::createMutexes(Signal * signal, Uint32 count){
  ...
  case 2:{
   ...
   case 3:{
    Mutex mutex(signal, c_mutexMgr, c_startLcpMutexHandle_KCJ);
    mutex.create(c);
    return;
  }
  }
...
{
printf("KCJ: Dbdih::execMASTER_LCPREQ\n");
  NodeRecordPtr newMasterNodePtr;
...
void Dbdih::initLcpLab(Signal* signal, Uint32 senderRef, Uint32 tableId) 
{
  ...
  if (c_lcpState.m_masterLcpDihRef != senderRef || c_lcpState.m_masterLcpDihRef != cmasterdihref)
  {
    /**
...
void Dbdih::packTableIntoPagesLab(Signal* signal, Uint32 tableId) 
{
  ...
  printf("KCJ: Dbdih::packTableIntoPagesLab() \n");
...
/*
 Our design of ScaleRDB performs independent transaction and checkpoint operations in master-slave pairs.

 1. Initialize to prevent all instances from participating in checkpoint and transaction operations.
	- participatingDIH.clear(): Clear to prevent all instances from participating in checkpoint operations.
	- participatingLQH.clear(): Clear to prevent all instances from participating in transaction operations.

 2. Map only the master instance and a single slave instance to participate in checkpoint and transaction operations.
	- The setEach function is implemented in Bitmask.hpp (ScaleRDB/storage/ndb/include/util).
	- req->participatingDIH.setEach(master instance ID, slave instance ID);
	- req->participatingLQH.setEach(master instance ID, slave instance ID);

 3. Send the START_LCP_REQ signal to the Transaction Manager (Macro) to request operations from the master instance and a single slave instance.
	- sendLoopMacro_KCJ(START_LCP_REQ, sendSTART_LCP_REQ, RNIL, 10[master instance], isNode_participating_num[slave instance]);

 4. Implement a round-robin manner to ensure that each transaction and checkpoint operation involves slave instances in the order of their IDs, allowing them to participate with the master instance.
	- After the last slave instance with the highest ID participates, reset the sequence so that the slave instance with the first ID can participate again.
	  if (isNode_participating_num == 13) { // last slave instance
         	isNode_participating_num = 11; // first slave instance
	  } else {
        	isNode_participating_num++; // slave instance ID + 1
	  }
...
*/
  ...
  sendLoopMacro_KCJ(START_LCP_REQ, sendSTART_LCP_REQ, RNIL, 10, isNode_participating_num);

  if (isNode_participating_num == 13) {
...
/*
void
Dbdih::startLcpMutex_unlocked_KCJ(Signal* signal, Uint32 data, Uint32 retVal){
  jamEntry();
  ndbrequire(retVal == 0);
...
void Dbdih::startNextChkpt(Signal* signal)
{
  ...
  StartLcpReq* req = (StartLcpReq*)signal->getDataPtrSend();
   ...
   printf("KCJ: Dbdih::startNextChkpt: c_lcpState.m_participatingLQH.count() > %llu\n", c_lcpState.m_participatingLQH.count()); 
...
void Dbdih::execLCP_FRAG_REP(Signal* signal) 
{
  ...
  case LMTOS_COPY_ONGOING:
    /**
     * In t3e old code we ensured that all outstanding LCP_FRAG_REPs
...
void Dbdih::setLcpActiveStatusStart(Signal* signal) 
{
  ...
  //c_lcpState.m_participatingLQH.set(11);
  //c_lcpState.m_participatingDIH.set(11);
