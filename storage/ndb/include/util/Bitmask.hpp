...
public:
  ...
  /*
  The ScaleRDB we designed operates as a master-slave instance pair. 
  To perform independent transactions and checkpoint operations at the pair level, we needed to implement a new data structure.  We modified the existing set() function, which used to delegate all tasks to all active master-slave instances, and created setEach(). 

 This new function(setEach()) allows parameters to be passed to only two selected database instances, enabling operations with one master instance and one slave instance.
